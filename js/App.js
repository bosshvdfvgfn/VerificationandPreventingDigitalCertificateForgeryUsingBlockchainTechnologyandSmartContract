window.CONTRACT = {
  address: '0xd572EbE17037CeCD3581f6d364a075f454E98824',
...
window.onload = async () => {
  ...
  if (window.ethereum) {
    ...
    if (window.userAddress.length > 10) {
      ...
      $('#loginButton').hide()
      $('#userAddress')
        .html(`<i class="fa-solid fa-wallet mx-2 text-primary"></i>${truncateAddress(
        window.userAddress,
      )}
      ...
      $('#Exporter-info').html(
        `<i class="fa-solid fa-building mx-2 text-warning"></i>${window.info}`,
      )
...
async function get_ethBalance() {
  await web3.eth.getBalance(window.userAddress, function (err, balance) {
    if (err === null) {
      $('#userBalance').html(
        "<i class='fa-solid fa-coins mx-2 text-danger'></i>" +
...
async function getCounters() {
  await window.contract.methods
    ...
    .then((result) => {
      $('#num-exporters').html(
        `<i class="fa-solid fa-building mx-2 text-info"></i>${result}`,
      )
    })
  await window.contract.methods
    .count_hashes()
